...
export default function StaffDashboard() {
  ...
  const orders = useMemo(() => {
    return allOrders
      .filter(o => o.status !== 'PLACED' && o.status !== 'CANCELLED')
      .slice(0, 4)
      .map(o => ({
        id: o.id,
        table: o.table,
  ...
  const getStatusConfig = (status: OrderStatus) => {
    switch (status) {
      case 'CONFIRMED':
        return { label: 'PENDING', bg: '#FEF3C7', color: '#D97706', icon: 'time' as const };
      case 'PREPARING':
        return { label: 'PREPARING', bg: '#DBEAFE', color: '#2563EB', icon: 'flame' as const };
      case 'READY':
        return { label: 'READY', bg: '#D1FAE5', color: '#059669', icon: 'checkmark-circle' as const };
      case 'SERVED':
      case 'BILLED':
      case 'PAID':
        return { label: 'SERVED', bg: '#F1F5F9', color: '#64748B', icon: 'checkmark-done' as const };
      default:
        return { label: status, bg: '#F1F5F9', color: '#64748B', icon: 'help-circle-outline' as const };
    }
  };
