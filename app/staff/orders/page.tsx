...
export default function Orders() {
  ...
  const allOrders = useMemo(
    () => orders.filter((order) => order.status !== 'PLACED' && order.status !== 'CANCELLED'),
    [orders],
  );

  const filters = [
    { id: 'all' as const, label: 'All', count: allOrders.length },
    { id: 'CONFIRMED' as const, label: 'Confirmed', count: allOrders.filter(o => o.status === 'CONFIRMED').length, color: '#f59e0b' },
