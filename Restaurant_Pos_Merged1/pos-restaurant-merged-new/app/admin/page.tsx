...
const DEF: RestoInfo = {
  name:'Restaurant', type:'Fine Dining', address:'', city:'',
  phone:'', description:'This portal puts every aspect of operations at your fingertips — live orders, staff, tables, and reports.',
};
...
export default function AdminLanding() {
  ...
  useEffect(() => {
    fetch('/api/admin/restaurant',{headers:{'Content-Type':'application/json'}})
      .then(r=>r.ok?r.json():null)
      .then(d=>{if(d)setResto({name:d.name||DEF.name,type:d.restaurant_type||DEF.type,address:d.address||'',city:d.city||'',phone:d.phone||'',description:d.description||DEF.description});})
      .catch(()=>{});
  },[]);
  ...
  useEffect(()=>{
    ...
    const t=setTimeout(()=>{
      setSplash('exit');
      setTimeout(()=>{setSplash('done');document.body.style.overflow='';},780);
    },3400);
  ...
  useEffect(()=>{
    ...
    document.querySelectorAll('a,button,.ic,.gi,.fc,.ab-stat,.sc-card').forEach(el=>{
  ...
  useEffect(()=>{
    const onSc=()=>{
      ...
      if(qSec.current&&qBg.current){
        ...
        qBg.current.style.transform=`translateY(${(window.innerHeight/2-r.top)/window.innerHeight*65}px)`;
      }
    };
  ...
  return (
    <div className="landing-root">
        ...
        <>
          ...
          {splash==='show'&&(
            <div className="splash">
              ...
              <div className="splash-corner sp-tl"/><div className="splash-corner sp-tr"/>
              <div className="splash-corner sp-bl"/><div className="splash-corner sp-br"/>
              <div style={{position:'relative',zIndex:2,textAlign:'center'}}>
                <div className="splash-name">{resto.name}</div>
                <div className="splash-rule"><div className="sr-l"/><div className="sr-dot"/><div className="sr-d"/><div className="sr-dot"/><div className="sr-l"/></div>
      ...
      <div className={`l-mob${mob?' open':''}`}>
        ...
        {['About','Operations','Gallery'].map(l=>(<a key={l} href={`#${l.toLowerCase()}`} onClick={()=>setMob(false)}>{l}</a>))}
      ...
      <nav className={`l-nav${sc?' sc':''}`}>
        ...
        <button className="ham" onClick={()=>setMob(true)} aria-label="Open menu"><b/><b/><b/></button>
      </nav>

      {/* HERO */}
      <div className="l-hero">
        <div className="hero-wm" aria-hidden="true">{initial}</div>
        <div className="hero-rules" aria-hidden="true">
          {[20,40,60,80].map(p=><span key={p} style={{top:`${p}%`}}/>)}
        </div>
        <div className="hero-body">
          <div className="hero-l">
            <div className="h-badge"><div className="h-badge-line"/><span className="h-badge-txt">Staff & Admin Portal</span></div>
            <h1 className="h-name">{resto.name}</h1>
            <p className="h-name-sub">{resto.type}</p>
            <div className="h-rule"><div className="h-rule-l"/><div className="h-rule-dot"/><div className="h-rule-d"/><div className="h-rule-dot"/><div className="h-rule-l"/></div>
            <p className="h-desc">Restaurant Management System<br/>{loc}</p>
            <a href="/admin/login" className="h-btn"><span>Access Dashboard</span><div className="h-btn-arrow"/></a>
            <p className="h-note">Authorised personnel only</p>
            <div className="h-scroll"><div className="hs-line"/><span className="hs-txt">Scroll</span></div>
          </div>
          <div className="hero-r">
            <div className="hero-img-top"><img src="https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=1400&q=85" alt="Dining room" loading="eager"/></div>
            <div className="hero-img-bot"><img src="https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=800&q=80" alt="Kitchen" loading="lazy"/></div>
            <div className="hero-yr"><div className="hero-yr-n">16</div><div className="hero-yr-t">Years of<br/>Excellence</div></div>
          </div>
        </div>
      </div>
      ...
      <div className="l-mq">
        <div className="mq-track">
          {mq3.map((t,i)=>(<div key={i} className="mq-item">{t}<span className="mq-sep">✦</span></div>))}
        </div>
      </div>
      ...
      <section id="about" className="l-about">
        <div className="rv-l">
          ...
          <p className="sec-p" style={{marginTop:10}}>Built for speed, precision, and ease — so your team can focus on what matters: delivering an unforgettable experience every service.</p>
      ...
      <section id="operations" className="l-imgscroll">
        ...
        <div className="scroll-row"><div className="scroll-inner sL">
          {r1d.map((c,i)=>(<div key={i} className="ic"><img src={c.img} alt={c.name} loading="lazy"/><div className="ic-ov"/><div className="ic-txt"><div className="ic-tag">{c.tag}</div><div className="ic-name">{c.name}</div></div></div>))}
        </div></div>
        <div className="scroll-row"><div className="scroll-inner sR">
          {r2d.map((c,i)=>(<div key={i} className="ic"><img src={c.img} alt={c.name} loading="lazy"/><div className="ic-ov"/><div className="ic-txt"><div className="ic-tag">{c.tag}</div><div className="ic-name">{c.name}</div></div></div>))}
        </div></div>
      </section>
      ...
      <section className="l-feat">
        ...
        <div className="feat-grid">
          {FEATS.map((f,i)=>(<div key={i} className="fc rv" style={{transitionDelay:`${i*0.07}s`}}><div className="fc-num">{f.num}</div><span className="fc-ico">{f.ico}</span><div className="fc-title">{f.title}</div><div className="fc-body">{f.body}</div></div>))}
        </div>
      </section>
      ...
      <section id="gallery" className="l-gallery">
        <div className="rv" style={{textAlign:'center'}}>
          ...
          <h2 className="sec-h" style={{textAlign:'center'}}>Inside <em>{resto.name}</em></h2>
        </div>
        <div className="gl-grid">
          {GALLERY.map((g,i)=>(<div key={i} className={`gi ${g.cls} rv`} style={{transitionDelay:`${i*0.09}s`}}><img src={g.src} alt={g.name} loading="lazy"/><div className="gi-ov"><div className="gi-ovname">{g.name}</div><div className="gi-ovtag">{g.tag}</div></div></div>))}
        </div>
      </section>
      ...
      <div className="l-stats">
        {STATS.map((s,i)=>(<div key={s.l} className="sc-card rv" style={{transitionDelay:`${i*0.1}s`}}><span className="sc-ico">{s.ico}</span><div className="sc-n">{s.n}</div><div className="sc-l">{s.l}</div></div>))}
      </div>
      ...
      <footer className="l-footer">
        ...
        <div className="ft-bottom">
          <span className="ft-copy">&copy; {new Date().getFullYear()} {resto.name}. All rights reserved.</span>
